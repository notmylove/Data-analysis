...
class ReverseTest:
    ...
    def nilian(self, arr_1, arr_2):
        for i in range(len(arr_1)-1, 2, -1):
            if np.abs(np.diff(arr_2[i - 3:i + 1], n=3)) < self.lamda:
                k1 = i - 3
                break
            elif i == 3:
                print('模型失效')
    ...
    def nijian(self, arr_1, arr_2):
        k = self.nilian(arr_1, arr_2)
            ...
            if np.abs(interpolate.UnivariateSpline(csth, csnh, k=3, s=0)(arr_1[i]) - arr_2[i]) < self.lamda:
                ...
                if len(list1) >= 10:
                    ...
                    if np.sum(np.diff(list1[-10:])) == 9:
                        ...
                        return np.hstack((self.nijian(arr_1[0:f], arr_2[0:f]), self.nijian(arr_1[f:], arr_2[f:])))
                        break
        return result1
...
def main():
    ...
    arr1 = data[0, :]
    arr2 = data[1, :]
    eliminating = ReverseTest(arr1, arr2)
    eliminating.spline_smoothing()
    result = eliminating.nijian(arr1, arr2)
    # 异常数据剔除后的效果图
